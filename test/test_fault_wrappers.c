...
static void write_outcome(const char *wrapper, const char *domain, const char *symbol, int code, int passed)
{
    ...
    {
        written = fprintf(outcome_stream, "P101WRAPPER\t1\tFAULT\t%s\tlib_c_facts\t%s\t%s\t%s\t%d\t%s\n", P101_TEST_PLATFORM, wrapper, domain, symbol, code, passed ? "PASS" : "FAIL");
...
/* P101_TEST_CASE(p101_c_analysis_scan) */
static void test_p101_c_analysis_scan(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        bool result = p101_c_analysis_scan(env, err, NULL, 0, argument_4);
        (void)result;
        ...
        write_outcome("p101_c_analysis_scan", "errno", error_names[index], state.code, failures == failures_before);
    ...
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            struct p101_c_analysis_options native_argument_2       = {0};
            unsigned char                  native_argument_4[4096] = {0};
            bool                           native_result           = p101_c_analysis_scan(native_env, native_err, &native_argument_2, 0, native_argument_4);
            (void)native_result;
            if(!p101_error_is_error(native_err, P101_ERROR_CHECK, -1))
            {
                fprintf(stderr, "native smoke did not produce the declared failure: p101_c_analysis_scan: %s\n", p101_error_get_message(native_err));
                native_passed = false;
            }
            if(native_result != false)
            {
                fprintf(stderr, "native smoke returned an undeclared result: p101_c_analysis_scan\n");
                native_passed = false;
            }
        ...
        if(native_pid > 0)
        {
            ...
            {
                fprintf(stderr, "native smoke terminated by signal: p101_c_analysis_scan: %d\n", WTERMSIG(native_status));
            }
            ...
            if(WIFEXITED(native_status))
            {
                ...
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_c_analysis_scan: %d\n", WEXITSTATUS(native_status));
                }
...
/* P101_TEST_CASE(p101_c_fact_parse_line) */
static void test_p101_c_fact_parse_line(struct p101_env *env, struct p101_error *err)
{
    char          argument_2[4];
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        errno                 = P101_TEST_ERRNO_SENTINEL;
        p101_env_set_fault_injector(env, fail_next_call, &state);
        enum p101_c_fact_status result = p101_c_fact_parse_line(env, err, argument_2, NULL);
        (void)result;
        EXPECT(state.checks == 1);
        EXPECT(p101_error_is_errno(err, state.code));
        EXPECT(errno == P101_TEST_ERRNO_SENTINEL);
        EXPECT(result == (P101_C_FACT_MALFORMED));
        EXPECT(memcmp(argument_2, argument_2_before, sizeof(argument_2)) == 0);
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_c_fact_parse_line", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            struct p101_c_fact      native_argument_3           = {0};
            enum p101_c_fact_status native_result               = p101_c_fact_parse_line(native_env, native_err, native_argument_2, &native_argument_3);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                fprintf(stderr, "native smoke failed: p101_c_fact_parse_line: %s\n", p101_error_get_message(native_err));
                native_passed = false;
            }
            native_child_status = native_passed ? EXIT_SUCCESS : EXIT_FAILURE;
        native_child_done_:
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_c_fact_parse_line: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_c_fact_parse_line: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
/* P101_TEST_CASE(p101_c_facts_find_clang_compile_database) */
static void test_p101_c_facts_find_clang_compile_database(struct p101_env *env, struct p101_error *err)
{
    char          argument_3[4];
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        errno                 = P101_TEST_ERRNO_SENTINEL;
        p101_env_set_fault_injector(env, fail_next_call, &state);
        bool result = p101_c_facts_find_clang_compile_database(env, err, NULL, argument_3, 0);
        (void)result;
        EXPECT(state.checks == 1);
        EXPECT(p101_error_is_errno(err, state.code));
        EXPECT(errno == P101_TEST_ERRNO_SENTINEL);
        ...
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_c_facts_find_clang_compile_database", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            bool native_result               = p101_c_facts_find_clang_compile_database(native_env, native_err, "p101", native_argument_3, 0);
            (void)native_result;
            if(!p101_error_is_error(native_err, P101_ERROR_CHECK, -1))
            {
                fprintf(stderr, "native smoke did not produce the declared failure: p101_c_facts_find_clang_compile_database: %s\n", p101_error_get_message(native_err));
                native_passed = false;
            }
            if(native_result != false)
            {
                fprintf(stderr, "native smoke returned an undeclared result: p101_c_facts_find_clang_compile_database\n");
                native_passed = false;
            }
            p101_error_reset(native_err);
            native_child_status = native_passed ? EXIT_SUCCESS : EXIT_FAILURE;
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_c_facts_find_clang_compile_database: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_c_facts_find_clang_compile_database: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
/* P101_TEST_CASE(p101_c_facts_with_compile_command) */
static void test_p101_c_facts_with_compile_command(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        errno                 = P101_TEST_ERRNO_SENTINEL;
        p101_env_set_fault_injector(env, fail_next_call, &state);
        bool result = p101_c_facts_with_compile_command(env, err, NULL, NULL, 0, argument_5);
        (void)result;
        EXPECT(state.checks == 1);
        EXPECT(p101_error_is_errno(err, state.code));
        EXPECT(errno == P101_TEST_ERRNO_SENTINEL);
        ...
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_c_facts_with_compile_command", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            bool          native_result           = p101_c_facts_with_compile_command(native_env, native_err, "p101", "p101", 0, native_argument_5);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                fprintf(stderr, "native smoke failed: p101_c_facts_with_compile_command: %s\n", p101_error_get_message(native_err));
                native_passed = false;
            }
            native_child_status = native_passed ? EXIT_SUCCESS : EXIT_FAILURE;
        native_child_done_:
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_c_facts_with_compile_command: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_c_facts_with_compile_command: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
int main(void)
{
    ...
    else
    {
        ...
        {
            test_p101_c_analysis_scan(env, err);
        }
        if(!native_child_process)
        {
            test_p101_c_fact_parse_line(env, err);
        }
        if(!native_child_process)
        {
            test_p101_c_facts_find_clang_compile_database(env, err);
        }
        if(!native_child_process)
        {
            test_p101_c_facts_with_compile_command(env, err);
        }
    }
